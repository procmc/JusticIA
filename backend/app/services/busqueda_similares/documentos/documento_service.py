...
class DocumentoService:
    ...
    async def obtener_expediente_completo(
        self, expedient_id: str, incluir_documentos: bool = True
        ...
        try:
            with self.get_db_session() as db:
                ...
                expedient_data = {
                    ...
                    "CN_Id_expediente": expediente.CN_Id_expediente,  # ID real de BD
                    "expedient_number": expediente.CT_Num_expediente,  # Número de expediente
                    "created_date": (
                        expediente.CF_Fecha_creacion.isoformat()
                        if expediente.CF_Fecha_creacion
                        else None
                    ),
                    ...
                    ),  # Usar fecha de creación como updated_date
                    "updated_by": None,
                    "documents": [],
                }
                ...
                if incluir_documentos:
                    expedient_data["documents"] = (
                        await self._obtener_documentos_expediente(
    ...
    async def _obtener_documentos_expediente(
        self, db: Session, expediente_id: int
    ) -> List[Dict[str, Any]]:
        """
        ...
        try:
            ...
            stmt = select(T_Expediente).where(
                T_Expediente.CN_Id_expediente == expediente_id
            )
            ...
            for doc in documentos:
                document = {
                    ...
                    "upload_date": (
                        doc.CF_Fecha_carga.isoformat() if doc.CF_Fecha_carga else None
                    ),
                    "content_preview": getattr(
                        doc, "CT_Contenido_preview", None
                    ),  # Puede no existir
                    "file_size": getattr(doc, "CN_Tamaño", None),  # Puede no existir
                    "file_type": doc.CT_Tipo_archivo,
                    "created_by": getattr(
                        doc, "CT_Creado_por", None
                    ),  # Puede no existir
                    "updated_date": (
                        doc.CF_Fecha_carga.isoformat() if doc.CF_Fecha_carga else None
                    ),  # Usar fecha de carga como updated_date
                }
                documents.append(document)

            logger.debug(
                f"Obtenidos {len(documents)} documentos para expediente ID {expediente_id}"
            )
            return documents

        except Exception as e:
            logger.error(
                f"Error obteniendo documentos del expediente ID {expediente_id}: {e}"
            )
            return []

    async def obtener_expedientes_batch(
        self, expedient_ids: List[str], incluir_documentos: bool = False
    ) -> List[Dict[str, Any]]:
        """
    ...
    async def verificar_existencia_expediente(self, expedient_id: str) -> bool:
        ...
        try:
            with self.get_db_session() as db:
                ...
                exists = self.expediente_repo.validar_expediente_existe(
                    db, expedient_id
                )
            ...
            logger.error(
                f"Error verificando existencia del expediente {expedient_id}: {e}"
            )
            return False
        ...
        try:
            with self.get_db_session() as db:
                ...
                return {
                    ...
                    "upload_date": (
                        documento.CF_Fecha_carga.isoformat()
                        if documento.CF_Fecha_carga
                        else None
                    ),
